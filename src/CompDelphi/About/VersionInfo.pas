...
TFileFlag = (vsDebug, vsInfoInferred,
              vsPatched, vsPreRelease,
              vsPrivateBuild, vsSpecialBuild);
...
constructor TVersionInfo.Create;
            ...
            s:= FvI.FileVersion;
            //copy( s, pos('.',s) );
            FileVersion.major:=StrToInt(copy(s, 0, pos('.',s)-1 ));
            s := copy(s, pos('.',s)+1,  Length( s ) - pos('.',s));
            FileVersion.minor:=StrToInt(copy(s, 0, pos('.',s)-1 ));
            s := copy(s, pos('.',s)+1,  Length( s ) - pos('.',s));
            FileVersion.release:=StrToInt(copy(s, 0, pos('.',s)-1 ));
            s := copy(s, pos('.',s)+1,  Length( s ) - pos('.',s));
            FileVersion.build:=StrToInt(s);

            FileVersion.asString :=  FvI.FileVersion;
