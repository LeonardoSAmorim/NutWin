...
type
  ...
  TToken=( variab, constant,
           ...
           square, third, fourth,
           FuncOneVar, FuncTwoVar, FuncThreeVar); //UNIFESP

  POperation = ^TCISParserOperation;
  ...
  TCISParserOperation = record
                 { MUST use pointers (!), because argument and destination are linked... }
                 Arg1, Arg2, Arg3 : PParserFloat; //UNIFESP- One more Arg.
                 Dest : PParserFloat;
  ...
  public
    ...
    procedure AddFunctionThreeParam(const AFunctionName: string; const Func: TMathProcedure); //UNIFESP
 ...
 function MYROUND(n, nd : ParserFloat) : ParserFloat;
...
Passed arguments / results:
  ...
  either the FunctionOne or the FunctionTwo or the new FunctionThree list:

  - First  argument --> arg1^
  - Second argument --> arg2^
  - Third argument --> arg3^ //UNIFESP - One more arg.
...
{***************************************************************}
{                                                               }
{          UNIFESP - IF Implementation as a Three Args function }
{                       (without short circuit)                 }
{                                                               }
{***************************************************************}

procedure _IF(AnOp: POperation); far;
begin
  with AnOp^ do
    if arg1^ <> 0 then
      dest^ := arg2^
...
// greater-than
procedure _GT(AnOp: POperation); far;
begin
  with AnOp^ do
    if arg1^ > arg2^ then
      dest^ := -1
...
// less-than
procedure _LT(AnOp: POperation); far;
begin
  with AnOp^ do
    if arg1^ < arg2^ then
      dest^ := -1
...
procedure _GTEQ(AnOp: POperation); far;
begin
  with AnOp^ do
    if arg1^ >= arg2^ then
      dest^ := -1
...
procedure _LTEQ(AnOp: POperation); far;
begin
  with AnOp^ do
    if arg1^ <= arg2^ then
      dest^ := -1
...
// equal
procedure _EQ(AnOp: POperation); far;
begin
  with AnOp^ do
    if arg1^ = arg2^ then
      dest^ := -1
...
// not-equal
procedure _NEQ(AnOp: POperation); far;
begin
  with AnOp^ do
    if arg1^ <> arg2^ then
      dest^ := -1
...
procedure _NOT(AnOp: POperation); far;
begin
  with AnOp^ do
    if arg1^ = 0 then
      dest^ := -1
...
procedure _AND(AnOp: POperation); far;
begin
  with AnOp^ do
    if ( arg1^ * arg2^ ) = 0 then
      dest^ := 0
...
procedure _OR(AnOp: POperation); far;
begin
  with AnOp^ do
    if ( arg1^ + arg2^ ) = 0 then
      dest^ := 0
...
procedure _MROUND(AnOp: POperation); far;
var
   x : ParserFloat;
begin
  with AnOp^ do
    ...
    begin
      ...
      begin
        case Token of
          ...
          FuncOneVar, FuncTwoVar, FuncThreeVar:    { job has been done in build already !};//UNIFESP
        end; {case}
...
// UNIFESP - Begin
  FunctionThree := TStringList.Create;
...
//UNIFESP - begin
procedure TCustomParser.AddFunctionThreeParam(const AFunctionName: string; const Func: TMathProcedure);
begin
  if IsValidIdent(AFunctionName) then
    FunctionThree.AddObject(UpperCase(AFunctionName), TObject(@Func))
  else
    raise EBadName.Create(AFunctionName);
end;
// UNIFESP - End
...
begin
  ...
  begin
    ...
    while NextOP <> nil do
      with NextOP^ do
      begin
        if (Arg1 = lastop^.Arg1) or (Arg1 = lastop^.Arg2) or (Arg1 = lastop^.Arg3) or (Arg1 = lastop^.Dest) then   //UNIFESP
          Arg1 := nil;

        if (Arg2 = lastop^.Arg1) or (Arg2 = lastop^.Arg2) or (Arg2 = lastop^.Arg3) or (Arg2 = lastop^.Dest) then   //UNIFESP
          Arg2 := nil;

        if (Arg3 = lastop^.Arg1) or (Arg3 = lastop^.Arg2) or (Arg3 = lastop^.Arg3) or (Arg3 = lastop^.Dest) then //UNIFESP
          Arg3 := nil;

        if (Dest = lastop^.Arg1) or (Dest = lastop^.Arg2) or (Dest = lastop^.Arg3) or (Dest = lastop^.Dest) then //UNIFESP
          Dest := nil;
    ...
    begin
      ...
      if IndexOfObject( TObject(Arg3)) >= 0 then Arg3 := nil; //UNIFESP
      if IndexOfObject( TObject(Dest)) >= 0 then Dest := nil;

      if (Dest <> nil) and (Dest <> Arg3) and (Dest <> Arg2) and (Dest <> Arg1) then //UNIFESP
        dispose(Dest);

      if (Arg3 <> nil) and (Arg3 <> Arg2) and (Arg3 <> Arg1) then //UNIFESP
        dispose(Arg3);
...
{.$ENDIF}
    ...
    AddObject('NOT', TObject(@_NOT)); // UNIFESP
...
{.$IFNDEF SpeedCompare}
    ...
    AddObject('RADIUS', TObject(@_radius));
    AddObject('LOGN', TObject(@_logN));
{.$ENDIF}

// UNIFESP - begin
    AddObject('GT', TObject(@_GT));
    AddObject('LT', TObject(@_LT));
    AddObject('GTEQ', TObject(@_GTEQ));
    AddObject('LTEQ', TObject(@_LTEQ));
    AddObject('EQ', TObject(@_EQ));
    AddObject('NEQ', TObject(@_NEQ));
    AddObject('AND', TObject(@_AND));
    AddObject('OR', TObject(@_OR));
    AddObject('ROUND', TObject(@_ROUND));
    AddObject('MROUND', TObject(@_MROUND));
// UNIFESP - end
