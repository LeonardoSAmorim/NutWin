...
type
  ...
  TRoundedButton = class(TGraphicControl)
  private
...
procedure Register;
begin
  RegisterComponents('Miscelanea', [TRoundedButton]);
end;
...
procedure TRoundedButton.CMFontChanged(var Message: TMessage);
begin
  invalidate;
end;
...
procedure TRoundedButton.GlyphChanged(sender: TObject);
begin
  invalidate;
end;

procedure TRoundedButton.MouseDown(Button: TMouseButton;
...
procedure TRoundedButton.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  FPushDown:=false;
  invalidate;
...
procedure TRoundedButton.SetColor(const Value: TColor);
begin
...
procedure TRoundedButton.SetFlat(const Value: boolean);
begin
  FFlat := Value;
end;

procedure TRoundedButton.SetGlyph(const Value: TBitmap);
begin
  FGlyph.assign(Value);
  if FGlyph<>nil then
  begin
...
procedure TRoundedButton.MakeGrayed;
...
procedure TRoundedButton.SetHotTrackColor(const Value: TColor);
begin
  FHotTrackColor := Value;
end;

procedure TRoundedButton.SetShape(const Value: TrbsShape);
begin
  FShape := Value;
  invalidate;
end;

procedure TRoundedButton.SetGray(const Value: boolean);
begin
  if FGray<>value then
  begin
   FGray := Value;
...
procedure TRoundedButton.SetTrackGlyph(const Value: boolean);
begin
...
procedure TRoundedButton.Loaded;
begin
  inherited;
  if FGlyph<>nil then
  begin
