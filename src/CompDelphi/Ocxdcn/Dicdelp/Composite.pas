...
type
  ...
  private
    ...
    Fm_Database : TDataBase;
    procedure Setm_Database( const Value : TDatabase );
  protected
    ...
    procedure VerificaOrdem;
  public
    { Public declarations }
    m_OwnDatabase : Boolean;
    m_UserPw,m_UserId : string;
    property m_Database : TDatabase read Fm_Database write Setm_Database;
...
begin
if Assigned (m_Database) and m_Database.Connected and m_OwnDatabase then
...
ORDER BY ATC_PF.codfilho_ATC;
     ...
     //SQL:= 'SELECT ' +  sFieldOneName + ' AS FIELDONE, ' +  sFieldTwoName + ' AS FIELDTWO FROM ' + sTableName + ' WHERE ' +  sFilter + ' ORDER BY ' + sSort;
     ...
     //Clausula ORDER BY
     //sSort := Sigla + '_PF.codfilho_' + Sigla;
     sSort :=')) ORDER BY ' + Sigla + '_PF.ORDEM_' + Sigla;  //+ ', ' + Sigla + '_PF.codfilho_' + Sigla;
...
if m_SemOrdem then
   ...
   begin
   Join.SQL.Add (sSelect + ', ' + m_FieldOrdem);
   Join.SQL.Add (sFrom);
   Join.SQL.Add (sFilter + '''' + Codigo + '''');
   Join.SQL.Add (sSort);
   m_SQLJoinPF_Flat := sSelect + ', ' + m_FieldOrdem + sFrom + sFilter + '''' + Codigo + ''''+ sSort;
   end;
...
procedure TComponentNode.VerificaOrdem;
begin
     ...
     m_SemOrdem := (m_PFHasCh.FieldDefs.IndexOf (m_FieldOrdem)=-1);
     m_PFHasCh.Close;
end;

procedure TComponentNode.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if Operation = opRemove then
  begin
    if (m_Database <> nil) and (AComponent = m_Database) then
       m_Database := nil;
  end;
end;

procedure TComponentNode.Setm_Database( const Value : TDatabase );
begin
   Fm_Database := Value;
