...
uses
    SysUtils, Classes, Forms, StdCtrls, DB, DBTables, dbpersis, DsgnIntf,
    Controls, UpdSQLEd,QBE, QBindDlg,DbXPlor,BDEProv,DSDesign, BDEConst;
type
  ...
  public
    { Public declarations }
    constructor Create (AOwner: TComponent;AUPOV : TUpdateObjectView);
  end;
...
var
  ...
  List: TParams;
begin
  Query := TUpdateObjectView(Component).DataSet as TQuery;
  case Index of
    0: ShowDatasetDesigner(Designer, TTable(Query));
    1:
      begin
        List := TParams.Create;
        try
    ...
    2: ExploreDataset(TDBDataset(Query));
    3:
      begin
        ExecBuilder(Query);
        if Designer <> nil then Designer.Modified;
      end;
    4: EditUpdateObjView (TUpdateObjectView(Component));
  end;
end;
...
begin
  case Index of
    0: Result := SSQLDatasetDesigner;
    1: Result := SBindVerb;
    2: Result := SExplore;
    3: Result := SQBEVerb;
    4: Result := 'Edit SQL Statements...'
  end;
end;
...
begin
  with UPDVParam do
       ...
       if not(EditUpdateSQL(UPDClone)) then
          begin
          DelStatement;
          exit;
          end;
