...
procedure Register;
begin
  RegisterComponents('DBPersistent', [TUpdateObjectView]);
  RegisterComponents('DBPersistent', [TDSPersist]);
  RegisterComponents('DBPersistent', [TQueryPersist]);
  RegisterComponents('DBPersistent', [TDSPersistRadioGroup]);
  RegisterComponents('DBPersistent', [TDSPersistCheckBox]);
  RegisterComponents('DBPersistent', [TDSPersistRButton]);
             ...
             for I := 0 to LkedDataObject.DataAncestor.DataSet.FieldCount - 1 do
                 ...
                 if Assigned (FindField (Campo)) then             
                           if (LkedDataObject.DataAncestor.DataSet.Fields[i].FieldKind =fkData) then
                               FieldValues [Campo]:= LkedDataObject.DataAncestor.DataSet.FieldValues[Campo];
                 end;
      ...
      for I := 0 to DataSet.FieldCount - 1 do
          ...
          if Assigned (FActiveDescendant.DataSet.FindField (Campo)) then
             if (DataSet.Fields[i].FieldKind =fkData) then
                 FActiveDescendant.DataSet.FieldValues [Campo]:= DataSet.FieldValues[Campo];
          end;
...
if not (ADescendant.DataSet.Locate((ADescendant.DataSet as TQueryPersist).CampoOUID,Campo,[])) then
   ...
   for I := 0 to DataSet.FieldCount - 1 do
       ...
       if Assigned (ADescendant.DataSet.FindField (Campo)) then
          if (DataSet.Fields[i].FieldKind =fkData) then
             ADescendant.DataSet.FieldValues [Campo]:= DataSet.FieldValues[Campo];
       end;
   ADescendant.FMoveKind:=mvNone;
...
if (Field=nil) and
   ...
   and (FMoveKind in [mvUp,mvNone]) then
      ...
      if Assigned (RefreshField) then
         RefreshField.Value:= Field.Value;
      (DataAncestor as TDSPersist).FMoveKind:=mvNone;
      end;
...
//showmessage (self.name + '-> DSPersist Loaded Begin');
     ...
     if not Assigned (FDataAncestor) then
         ...
         if Assigned (AnAncestor.DataSet)  and (AnAncestor.DataSet.State <> dsInactive) then
            AnAncestor.DataSet.First;
