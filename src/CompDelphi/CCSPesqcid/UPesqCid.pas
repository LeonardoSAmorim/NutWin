...
uses
  ...
  registry,Forms, clipbrd, Dialogs, Grids, StdCtrls, Buttons, ExtCtrls, Index32, ComCtrls;
...
type
  ...
  TBuscaCodigo = class(TForm)
    ...
    procedure MemorizaPalavra ( NovaPalavra : string);
 ...
 protected
  ...
  protected
    ...
    FBitMap : TBitMap;
    procedure LoadControlBitmap;virtual;
  public
    { Public declarations }
...
function TBuscaCodigo.GetToken(var S: string): string;
var  I: integer;
     Temp: string;
begin
  I := pos(',',S);
...
function TBuscaCodigo.InitDic:boolean;
var
I:integer;
AuxStr : string;
Reg: TRegistry;
begin
    ...
    try
      begin
        ...
        if Reg.OpenKey('\CLSID\{A8A4FAA5-8936-11D3-8576-006008DF8A1A}\InprocServer32',False) then
           rota:=copy (Reg.ReadString(''),1,LastDelimiter('\',Reg.ReadString('')))
        else
  ...
  ArquivoIni := TMemIniFile.Create(rota + 'ccssis.ini');

  cbChave.Items.Clear;
  for I:=0 to 9 do
      begin
      AuxStr:=ArquivoIni.ReadString ('PESQCID','Pesquisa'+IntToStr(I),' ');
      if AuxStr <> ' ' then
         cbChave.Items.Add(AuxStr);
      end;
...
function TBuscaCodigo.PegaPalavra(var S,Palavra:String):boolean;
begin
  Result := true;
...
procedure TBuscaCodigo.ANDLogico(var A,B,Temp : TipoOcorr);
var
  IndA,IndB : integer;
begin
  Temp[0] := 0;
...
procedure TBuscaCodigo.ORLogico(var A,B,Temp : TipoOcorr);
var
  IndA,IndB : integer;
begin
  Temp[0] := 0;
...
procedure TBuscaCodigo.CarregaOcorr(var EsteIndice: integer; var EstaOcorr:tipoOcorr);
begin
...
procedure TBuscaCodigo.PegaAsterisco(Palavra:string; var Saida:tipoOcorr);
var
  ListOcorr : pOcorr;
  I         : integer;
begin
...
function TBuscaCodigo.SemAcento(S:String):String;
var
  Temp : String;
  I    :integer;
begin
...
function TBuscaCodigo.PegaTexto(Desloc:integer):string;
var
  Temp  : string;
  Lidos : integer;
begin
...
begin
  ...
  if not chbxPAutomatica.Checked then
     MemorizaPalavra (cbChave.Text);
...
function TBuscaCodigo.minimo(Pi,Pe:pchar):pchar;
begin
  TemBarra := true;
  Prox := false;
...
procedure TBuscaCodigo.ChaveDupla(PosTab,PosSep,Ch1,Ch2:integer;
...
if chbxPAutomatica.Checked then
   ...
   bProcura.Enabled :=False;
   bAtivaDicionario.Enabled := False;
   end
else
   begin
   cbChave.OnChange := nil;
   bProcura.Enabled :=True;
   bAtivaDicionario.Enabled := True;
   end;
end;
...
procedure TBuscaCodigo.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if Key=VK_F1 then
btHelpClick(self);
...
procedure TBuscaCodigo.btHelpClick(Sender: TObject);
var
hlpstr : PChar;
begin
hlpstr:=PChar(Rota + 'PesqCid.hlp');
WinHelp (Handle,hlpstr,HELP_KEY,DWORD(PChar('TelaPesquisa')));
end;
...
procedure TBuscaCodigo.MemorizaPalavra(NovaPalavra: string);
var
I : integer;
begin
    if cbChave.Items.IndexOf (NovaPalavra) >= 0 then
       exit;
    cbChave.Items.Insert (0,NovaPalavra);
    if cbChave.Items.Count > 10 then
       cbChave.Items.Delete(cbChave.Items.Count-1);
    ArquivoIni.EraseSection ('PESQCID');

    for I:=0 to cbChave.Items.Count - 1 do
        ArquivoIni.WriteString ('PESQCID','Pesquisa'+IntToStr(I),cbChave.Items[I]);
end;

procedure TBuscaCodigo.FormClose(Sender: TObject;
...
begin
   ...
   with PesqForm do
        begin
        //Acerta a rota e o nome do arquivo primeiro,
        //pois o InitDic precisa
        Rota:=FDiretorioCID;
        NomeArq:=FArquivosCID;
        ...
        WindowState := FFormWindowState;
        Position := FFormPosition;

        chbxPAutomatica.Checked:=FPesquisaAutomatica;
        chbxPAutomaticaClick (self);

        cbChave.Text := FPesquisaInicial;
        bProcura.Click;
        end;
 ...
 if PesqForm.ModalResult = mrOk then
    begin
      ...
      if CanClose then
         ...
         FDescricaoCID:= PesqForm.TextDescrCID;
...
begin
FBitMap.LoadFromResourceName(HInstance, 'TPESQCIDX');
end;

procedure TBuscaCodigo.btCopiarClick(Sender: TObject);
var
Texto : string;
begin
Texto :=Lista.Items[Lista.ItemIndex];
...
procedure TPesqCid.SetArquivosCID(const Value: string);
begin
  FArquivosCID := Value;
end;

procedure TPesqCid.Paint;
{
...
procedure TPesqCid.WMSize(var Message: TWMSize);
begin
