...
uses
  ...
  qrepform, Measurement, jpeg, CalcAli, RelConfig;

type
  TfmRelItensAli = class(TFormReport)
...
var
  fmRelItensAli: TfmRelItensAli;

implementation

{$R *.DFM}

procedure TfmRelItensAli.SetCalculoAlimentar(
...
procedure TfmRelItensAli.ReportBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  inherited;
  ...
  if not Assigned( FCalculoAlimentar ) then
     exit;
  taMedida.DatabaseName := FCalculoAlimentar.Database.DatabaseName;
  taMedida.Open;
  quRefItemsAli.ParamByName( 'ID_CALCALI' ).AsString := FCalculoAlimentar.IDCalcAli;
  quRefItemsAli.DatabaseName := FCalculoAlimentar.Database.DatabaseName;
  quRefItemsAli.Open;
end;

procedure TfmRelItensAli.Notification(AComponent: TComponent;
...
function TfmRelItensAli.GetIDReport: String;
begin
   Result := '{83294A6D-04B5-11D4-9DBF-000021609D7C}';
end;

procedure TfmRelItensAli.qbSumarioAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  inherited;
...
procedure TfmRelItensAli.qrCabecalhoBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TfmRelItensAli.SetRelConfig(const Value: TRelatorio);
begin
   if (Value <> nil) then
   begin
      FRelConfig := Value;
