...
uses
  ...
  ExtCtrls, QRPRNTR, fmRelViewer, RelConfig, PRINTERS;

type
  TfmRelIdentificacaoLandscape = class(TFormRepIndividuo)
...
var
  fmRelIdentificacaoLandscape: TfmRelIdentificacaoLandscape;

implementation

{$R *.DFM}

procedure TfmRelIdentificacaoLandscape.Notification(AComponent: TComponent;
...
procedure TfmRelIdentificacaoLandscape.FormClose(Sender: TObject;
...
procedure TfmRelIdentificacaoLandscape.FormCreate(Sender: TObject);
begin
  inherited;
...
function TfmRelIdentificacaoLandscape.GetIDReport: String;
begin
   Result := '{C523C5A8-04B3-11D4-9DBF-000021609D7C}';
end;

procedure TfmRelIdentificacaoLandscape.SetVisorMedidaNumDiasInquerito( NomeMedida : String );
begin
...
procedure TfmRelIdentificacaoLandscape.ReportPreview(Sender: TObject);
begin
  fmPreview.QRPreview.QRPrinter := TQRPrinter(Sender);
  fmPreview.ShowModal;
end;

procedure TfmRelIdentificacaoLandscape.qbSumarioAfterPrint(
...
procedure TfmRelIdentificacaoLandscape.SetRelConfig(
  const Value: TRelatorio);
begin
   if (Value <> nil) then
   begin
      FRelConfig := Value;
...
procedure TfmRelIdentificacaoLandscape.ReportBeforePrint(
...
procedure TfmRelIdentificacaoLandscape.ConfigDefault;
begin
     FRelConfig := TRelatorio.Create(self);
     with FRelconfig do
     begin
        Descricao := 'Identificação (Landscape)';
