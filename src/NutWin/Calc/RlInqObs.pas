...
type
  TfmRelInqObservacoes = class(TFormReport)
    qrCabecalho: TQRBand;
    qbInqObs: TQRBand;
    qeInqObs: TQRDBRichText;
    ...
    procedure qbSumarioBeforePrint(Sender: TQRCustomBand;
      var PrintBand: Boolean);
  ...
  private
    ...
    FCalculoAlimentar: TCalculoInquerito;
    FRelConfig : TRelatorio;
    procedure SetCalculoAlimentar(const Value: TCalculoInquerito);
    { Private declarations }
  public
    { Public declarations }
    property CalculoAlimentar : TCalculoInquerito read FCalculoAlimentar write SetCalculoAlimentar;
    function GetIDReport : String; override;
    procedure SetRelConfig( const Value : TRelatorio ); override;
  end;

var
  fmRelInqObservacoes: TfmRelInqObservacoes;
...
{ TfmRelInqObservacoes }

function TfmRelInqObservacoes.GetIDReport: String;
begin
   Result := '{FFBB5689-20F9-11D4-9DBF-000021609D7C}';
end;

procedure TfmRelInqObservacoes.Notification(AComponent: TComponent;
...
procedure TfmRelInqObservacoes.SetCalculoAlimentar(
...
procedure TfmRelInqObservacoes.FormCreate(Sender: TObject);
begin
  inherited;
  with qeInqObs do
  begin
     DataSet := dmMotherBoard.dsInqObservacoes.DataSet;
     DataField := 'OBSERVACOES';
  end;
end;

procedure TfmRelInqObservacoes.ReportBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  inherited;
...
procedure TfmRelInqObservacoes.qrCabecalhoBeforePrint(
...
procedure TfmRelInqObservacoes.SetRelConfig(const Value: TRelatorio);
begin
   if (Value <> nil) then
   begin
      FRelConfig := Value;
...
procedure TfmRelInqObservacoes.qbSumarioBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TfmRelInqObservacoes.ReportNeedData(Sender: TObject;
