...
type
  TfmRelPorcEnergia = class(TFormReport)
    qbMacroNut: TQRBand;
    qrCabecalho: TQRBand;
    ...
    procedure qbSumarioAfterPrint(Sender: TQRCustomBand;
      BandPrinted: Boolean);
    procedure qbMacroNutAfterPrint(Sender: TQRCustomBand;
      BandPrinted: Boolean);
  protected
...
var
  fmRelPorcEnergia: TfmRelPorcEnergia;

implementation

{$R *.DFM}

procedure TfmRelPorcEnergia.FormClose(Sender: TObject;
...
begin
  ...
  TAtivaCalculoAlimentar( FCalculoAlimentar.Ativar ).PorcentagemEnergiaCalculada := False;
end;

procedure TfmRelPorcEnergia.SetCalculoAlimentar(
...
procedure TfmRelPorcEnergia.ReportBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  inherited;
  ...
  TAtivaCalculoAlimentar( FCalculoAlimentar.Ativar ).PorcentagemEnergiaCalculada := True;
  with TCalculoAlimentar(FCalculoAlimentar).PorcentagemEnergiaCalculada do
  begin
     qtAlcool.DataSet := DataSet;
     qtAlcool.DataField := 'PORC_ENERGIA_ALCOOL';
     qtAgSat.DataSet := DataSet;
     qtAgSat.DataField := 'PORC_ENERGIA_AGSAT';
     qtAgMon.DataSet := DataSet;
     qtAgMon.DataField := 'PORC_ENERGIA_AGMON';
     qtAgPol.DataSet := DataSet;
     qtAgPol.DataField := 'PORC_ENERGIA_AGPOL';
  end;
end;

procedure TfmRelPorcEnergia.Notification(AComponent: TComponent;
...
function TfmRelPorcEnergia.GetIDReport: String;
begin
   Result := '{83294A70-04B5-11D4-9DBF-000021609D7C}';
end;

procedure TfmRelPorcEnergia.SetRelConfig(const Value: TRelatorio);
begin
   if (Value <> nil) then
   begin
      FRelConfig := Value;
...
procedure TfmRelPorcEnergia.qrCabecalhoBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TfmRelPorcEnergia.qbSumarioAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  inherited;
...
procedure TfmRelPorcEnergia.qbMacroNutAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  inherited;
